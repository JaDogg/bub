...
static void PluginProcessEvent(MyPlugin *plugin, const clap_event_header_t *event) {
    if (event->space_id == CLAP_CORE_EVENT_SPACE_ID) {
        if (event->type == CLAP_EVENT_NOTE_ON || event->type == CLAP_EVENT_NOTE_OFF || event->type ==
...
static void PluginPaintRectangle(MyPlugin *plugin, uint32_t *bits, uint32_t l, uint32_t r, uint32_t t, uint32_t b,
...
static void PluginPaint(MyPlugin *plugin, uint32_t *bits) {
    ...
    PluginPaintRectangle(plugin, bits, 10, 40, 10 + 30 * (1.0f - plugin->mainParameters[P_VOLUME]), 40, 0x000000,
                         0x000000);
}
...
static const clap_plugin_descriptor_t pluginDescriptor = {
    ...
    .name = "HelloCLAP",
    .vendor = "bub",
    .url = "https://jadogg.github.io",
    .manual_url = "https://jadogg.github.io",
    .support_url = "https://jadogg.github.io",
    .version = "1.0.0",
...
static const clap_plugin_t pluginClass = {
    ...
    .init = [](const clap_plugin *_plugin) -> bool {
        ...
        plugin->hostPOSIXFDSupport = (const clap_host_posix_fd_support_t *) plugin->host->get_extension(
            plugin->host, CLAP_EXT_POSIX_FD_SUPPORT);
        plugin->hostTimerSupport = (const clap_host_timer_support_t *) plugin->host->get_extension(
    ...
    .get_extension = [](const clap_plugin *plugin, const char *id) -> const void *{
        ...
        std::cout << "extension not supported: " << id << std::endl;
        return nullptr;
    },
